...
import { formatDateLong, formatMoney } from "../services/format.js";
...
export function bookingPaymentSummaryHtml(booking: Booking): string {
  ...
  return `
    <div><strong>${tableName}</strong> — ${formatDateLong(booking.date)}</div>
    <div>Party of ${booking.party_size} under "${booking.guest_name}"</div>
    <div style="margin-top:10px;">Minimum spend: ${formatMoney(minSpendCents)}</div>
    <div><strong>Deposit due now: ${formatMoney(depositCents)}</strong></div>
    <div style="color:#666;">Balance of ${formatMoney(balanceCents)} settled at the table on the night</div>
  `;
}
